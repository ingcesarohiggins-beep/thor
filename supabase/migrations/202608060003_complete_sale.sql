-- Confirma una venta como una única transacción: venta, pagos, stock, movimiento y auditoría.
...
begin
  ...
  if not exists (
    ...
    where id = p_seller_id and auth_user_id = auth.uid() and location_id = p_location_id and active = true
  ) then
